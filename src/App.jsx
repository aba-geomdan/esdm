...
// true: 미리보기용(샘플 JAR 즉시 표시). 배포 시 false 로 바꿔 실제 AI 생성 사용.
const DEMO_MODE = false;

// Supabase Edge Function 릴레이 URL (기존 프로젝트 재사용)
const RELAY_URL =
...
goals에는 위 ESDM 커리큘럼 영역(${
  ...
  function buildDemoJAR() {
    ...
    const goalTpl = {
      ...
      놀이: (items) =>
        `${items.join("·")} — ${main}으(로) 같은 공간에서 함께·번갈아 놀기`,
      인지: (items) =>
    ...
    const demo = {
      ...
      theme: {
        ...
        scenes: [
          {
            label: "장면 1: 관심 끌기",
            parent: `${main}을(를) 손에 들고 "짠~! ${main} 나왔다~" 하며 아이 눈높이에서 흔들어 보여줍니다`,
            strategy:
              "아이가 쳐다보면 3~5초 기다린 뒤 건네며 교대의 첫 경험을 만듭니다. 손을 뻗으면 즉시 \"줘?\" 하고 언어 모델 제공",
          },
          {
            label: "장면 2: 함께 다루기",
            parent: `아이와 같이 ${main}을(를) 만지며 "조물조물~ ${main} 만져!" 하고 동작을 크게 보여줍니다`,
            strategy:
              "아이의 동작을 똑같이 모방해 주며 \"OO이도 조물조물!\" 언어 모델. 같은 동작 모방으로 사회적 연결 강화",
          },
          {
            label: "장면 3: 주고받기",
            parent: `"엄마 줘~" 하고 손을 내밀고, 받으면 "고마워!" 하며 "이번엔 OO이 줄게~" 다시 건넵니다`,
            strategy: `건넬 때마다 "줘", "여기" ${wordLevel} 지시를 반복. 아이가 건네면 과장된 표정으로 "우와!" 즉각 반응하여 강화`,
          },
          {
            label: "장면 4: 변화 주기",
            parent: `${sub}을(를) 쓱 꺼내 "어? 이번엔 ${sub}이다!" 하며 놀란 표정으로 새 자극을 더합니다`,
            strategy: `예측을 살짝 깨뜨려 아이의 주의를 다시 모읍니다. 아이가 반응하면 ${sub}을(를) 가리키며 "여기 봐!" 공동주의 유도`,
          },
          {
            label: "장면 5: 신나게 절정",
            parent: `${main}와(과) ${sub}을(를) 함께 움직이며 "슈웅~ 출발!" 하고 과장되게 "와~!" 박수칩니다`,
            strategy:
              "감정을 크게 표현해 즐거움을 공유합니다. 아이가 웃거나 소리내면 \"또 할까?\" 하고 3초 기다려 자발적 요청 기다리기",
          },
          {
            label: "장면 6: 차례 지키기",
            parent: `"엄마 차례~" 한 번, "OO이 차례~" 한 번 번갈아 하며 ${main}을(를) 주고받습니다`,
            strategy:
              "\"차례\" 개념을 짧은 대사로 반복. 아이가 차례를 기다리면 즉시 \"잘 기다렸어!\" 칭찬으로 사회기술 강화",
          },
          {
            label: "장면 7: 마무리 정리",
            parent: `"이제 쏙~ 넣자!" 하며 ${main}을(를) 통이나 상자에 넣는 동작을 보여줍니다`,
            strategy:
              "아이 손에 쥐어 주고 통을 가리키며 \"넣어\" 지시. 넣으면 \"우와, 다 했다!\" 즉각 칭찬으로 자조·마무리 경험 제공",
          },
        ],
      },
      variations: [
